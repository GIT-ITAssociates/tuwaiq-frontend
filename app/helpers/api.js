...
export async function postFormMultiple(url, data, config = {}) {
  axiosApi.defaults.headers.common["Authorization"] =
    `Bearer ${localStorage.getItem("token") ?? ""}`;

  // ❌ Don't set Content-Type manually, let Axios set the boundary correctly
  delete axiosApi.defaults.headers.common["Content-Type"];

  let formData;

  if (data instanceof FormData) {
    // already a FormData, don’t convert again
    formData = data;
  } else {
    // convert plain object to FormData
    formData = convertObjectToFormData(data);
  }
