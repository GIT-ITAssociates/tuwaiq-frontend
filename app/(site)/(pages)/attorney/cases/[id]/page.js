...
const BookingDetails = ({ params }) => {
  ...
  const [clients] = useFetch(getAllClients);
  ...
  // modal + attachment picking
  const [imageModal, setImageModal] = useState(false);
  const [attachMode, setAttachMode] = useState(null); // 'image' | 'pdf' | null

  // attachment state (picked, not yet uploaded)
  const [imagePreview, setImagePreview] = useState(""); // tiny preview URL for image
  const [selectedImageFile, setSelectedImageFile] = useState(null);
  const [selectedPdfFile, setSelectedPdfFile] = useState(null);
  const [filelist, setFileList] = useState([]); // for UploadFileComponent

  const i18n = useI18n();

  useEffect(() => {
    if (params?.id) getData({ _id: params?.id });
  }, [params?.id]);
  ...
  const handleSubmit = async () => {
    try {
      const res = await updateCaseRequest({ _id: data?._id, status: "accept" });
      if (res?.error) {
  ...
  // Upload (if needed) + send note on main "Send" button
  const handleSendNotes = async (e) => {
    e.preventDefault();

    if (!notes && !selectedImageFile && !selectedPdfFile) {
      message.warning(i18n?.t("Please write a note or attach a file."));
      return;
    }
    ...
    try {
      ...
      if (attachMode === "image" && selectedImageFile) {
        const { error, data: imgUrl } = await postSingleImage({
          image: selectedImageFile,
          image_name: "notes",
        });
        if (!error && imgUrl) documentUrl = imgUrl;
      }

      // upload pdf if picked
      if (attachMode === "pdf" && selectedPdfFile) {
        const { error, data: pdfUrl } = await pdfFileUpload({
          files: selectedPdfFile,
          pdf_name: "notes",
        });
        if (!error && pdfUrl) documentUrl = pdfUrl;
      }

      const { error } = await postNotes({
        case_id: data?._id,
        note: notes || "",
      ...
      if (error) {
        message.error(i18n?.t("Failed to add note."));
        return;
      }

      message.success(i18n?.t("Note added successfully!"));
      ...
      message.error(i18n?.t("Something went wrong. Please try again."));
    } finally {
      setIsSending(false);
    }
  };
  ...
  return (
    <div className=" xl:pb-0 pb-[20px]">
      ...
      <div className="md:px-10 px-5 md:mt-10 mt-6 md:mb-[117px] mb-10">
          ...
          <div>
            <div className="border rounded-[20px] lg:px-10 px-5 lg:pt-10 lg:pb-[40px]  py-5 h-full">
              ...
              <div>
                ...
                <p className="font-medium work-sans mb-[30px]">
                  <span className="text-[#818B8F]">{i18n?.t("Hearing Date")}:</span>{" "}
                  {date ? date : "N/A"}
                </p>
        ...
        {data?.status === "pending" && data?.from === "user" && (
...
// const [selectedImageFile, setSelectedImageFile] = useState(null);
// const [selectedPdfFile, setSelectedPdfFile] = useState(null);
// const [filelist, setFileList] = useState([]);                 // for PDF Upload component
...
//           }
//           e.preventDefault()
//           const { error, msg } = await postNotes({ to: activeChatId, message: notes })
//           if (error === false) {
//               setNotes('')
//               getUsers()
//           }
...
//                     {dayjs(data?.appointment?.select_date).format(
//                       "DD MMMM YYYY"
//                     )}
//                   </p>
...
//         {/* This is the new cardview for showing a list of attached notes and file (file should be one). */}
//         {/* Case Notes Section */}
// <div className="border rounded-[20px] lg:px-10 px-5 lg:py-[40px] py-5 min-h-[237px] mt-6">
//   <h1 className="font-medium text-2xl leading-[28px] text-[#242628] work-sans mb-10">
//     {i18n?.t("Case Notes")}
...
//         // ✅ send note + optional document
//         if (notes || documentUrl) {
//           const { error } = await postNotes({
//             case_id: data?._id,
//             note: notes || "",
//             document: documentUrl || "",
...
//             imageForm.resetFields();
//             setImageModal(false);
//             getData(); // refresh notes list
//           } else {
//             message.error(i18n?.t("Failed to add note."));
//           }
//         } else {
//           message.warning(i18n?.t("Please add a note or attach a file."));
//         }
//       } catch (err) {
...
//       {/* Attachment picker (switchable; always enabled) */}
//       {attachMode === "pdf" ? (
//         <UploadFileComponent
//           label={i18n?.t("Choose PDF file")}
//           name="pdf"
...
//           setFileList={(list) => {
//             // only keep one file
//             const one = list?.length > 0 ? [list[list.length - 1]] : [];
//             setFileList(one);
//             // also keep selectedPdfFile in sync
//             const f = one?.[0]?.originFileObj;
//             if (f) {
//               setSelectedPdfFile(f);
//               // if user switched from image, clear preview
//               setSelectedImageFile(null);
//               setImagePreview("");
//               imageForm.resetFields(["image"]);
//             }
...
//           name="image"
//           className="w-full"
//           // if your component supports it, enforce single:
//           maxCount={1}
//           onChange={(files) => {
//             const f = files?.[0]?.originFileObj || files?.[0];
//             if (f) {
...
//       {/* Tiny image preview with remove/replace */}
//       {imagePreview && attachMode === "image" && (
...
//       {/* Simple chip for PDF with remove/replace */}
//       {attachMode === "pdf" && filelist?.[0]?.name && (
//         <div className="mt-2 flex items-center gap-3">
//           <span className="text-sm bg-gray-100 border px-2 py-1 rounded">
//             {filelist[0].name}
//           </span>
//           <button
//             type="button"
